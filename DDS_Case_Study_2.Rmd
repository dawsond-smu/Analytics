...
CaseStudy = CaseStudyNoSalaryNoAttrition %>%  mutate(JobInvolvement = as.factor(JobInvolvement), 
...
head(CaseStudy_Omit)
summary(CaseStudy_Omit)
glimpse(CaseStudy_Omit)
skim(CaseStudy_Omit)
DataExplorer::create_report(CaseStudy)
DataExplorer::create_report(CaseStudy_Omit)
...
fm1 <- lm(MonthlyRate ~ MaritalStatus + Over18, data = CaseStudy_Omit)
dd <- dredge(fm1)
subset(dd, delta < 4)
...
```{r}
models <- regsubsets(MonthlyRate ~ ., data = CaseStudy_Omit)
summary(models)
